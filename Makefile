...
build-debug build-release: build-%: cmake-%
	@cmake --build build_$* -j $(NPROCS) --target userver_sample

# Test
.PHONY: test-debug test-release
test-debug test-release: test-%: build-%
	@cmake --build build_$* -j $(NPROCS) --target userver_sample_unittest
	@cmake --build build_$* -j $(NPROCS) --target userver_sample_benchmark
...
service-start-debug service-start-release: service-start-%: build-%
	@cd ./build_$* && $(MAKE) start-userver_sample
...
install-debug install-release: install-%: build-%
	@cd build_$* && \
		cmake --install . -v --component userver_sample
